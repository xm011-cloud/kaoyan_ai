...
| 测试 | Playwright 144 用例 · 独立测试库 `neondb_test` | ^1.61.1 |
...
### 第 31 轮 — 学习行为驱动长期路线（2026-09-12，已部署）
...
### 第 32 轮 — 学习闭环可信度补强（2026-09-14，待发布）
- **周复盘先看事实**：周报新增本周计划健康度，单列已打卡时长、任务推进与课程/练习/错题证据；同一事实进入 AI 周报上下文，不重复累计不同来源时长。
- **课程笔记可继续使用**：课时内笔记支持分类筛选、编辑与“带着这一条问 AI”；笔记始终保留与课程和学习会话的关联。
- **弱网课程现场可恢复**：进行中的课时会将笔记、自评、卡点和下一步保存在本地；离线结束或暂存笔记会在联网后幂等补传，证据只在服务端确认后才计入路线，登出会清除本地草稿。Service Worker 仅缓存公开导航与静态资源，不会跨账号回放学习页。
- **校准不会永久有效**：超过 28 天的掌握度确认提示复盘；后续计划先安排巩固/复述验证，但不自动降档或阻断学习。
- **资料引用诚实化**：没有可提取文字的扫描 PDF、图片、Word 与未支持格式不会进入 RAG 或成为引用来源；资料页明确显示 AI 是否可检索。
...
| P2 | PWA 离线读+写队列已落地（2026-08-16：静态/GET API 缓存兜底 + IndexedDB 写入队列自动补传）；课程会话与草稿落盘已完成。后续：离线包预缓存 |
