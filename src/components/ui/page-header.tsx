...
export function PageHeader({
  ...
  return (
    <div className={cn('flex flex-wrap items-end justify-between gap-4 max-sm:flex-col max-sm:items-stretch', className)}>
      <div className="min-w-0">
        <h1 className="text-2xl font-semibold tracking-[-0.035em] sm:text-3xl">{title}</h1>
        {subtitle && <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{subtitle}</p>}
      </div>
      {action && <div className="shrink-0 max-sm:w-full max-sm:[&>*]:min-h-11 max-sm:[&>*]:w-full">{action}</div>}
    </div>
  )
}
